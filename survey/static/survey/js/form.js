...
function submit_feedback_form($form) {
    ...
    if (!hasMessage && !hasEmail) {
        $('#success-image').hide();
        $('#success-message').text('Спасибо за участие в опросе! Ваш код: — вы можете скопировать и вставить его в толоку для получения денег.');
    } else if (!hasMessage) {
        $('#success-message').text('Мы получили ваш адрес почты. Спасибо! Ваш код: — вы можете скопировать и вставить его в толоку для получения денег.');
    }
    send_ajax($form);
}
